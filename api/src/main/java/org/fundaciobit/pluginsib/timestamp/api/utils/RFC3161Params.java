...
public final class RFC3161Params {
     ...
     *                          */
    public RFC3161Params(final boolean requireCert, final String policy, final URI url, final String usr,
            final String pwd, final RFC3161RequestExtension[] exts, final String hashAlgorithm,
            final byte[] sslKeyStoreFile, final String sslKeyStorePassword, final String sslKeyStoreType,
            final byte[] sslTrustStore, final String sslTrustStorePassword, final String sslTrustStoreType,
    ...
    public RFC3161Params(final Properties extraParams) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Se ha indicado una URL de TSA invalida (" + tsa + "): " + e, e //$NON-NLS-1$ //$NON-NLS-2$
            );
        }
        ...
        if (keyStoreDataB64 != null) {
            try {
                ...
                throw new IllegalArgumentException(
                        "No se ha proporcionado en el parametro 'tsaSslKeyStore' el almacen de claves del SSL de la TSA en base 64: " //$NON-NLS-1$
                                + e,
                        e);
            }
        } else {
        ...
        if (trustStoreDataB64 != null) {
            try {
                ...
                throw new IllegalArgumentException(
                        "No se ha proporcionado en el parametro 'tsaSslTrustStore' el almacen de confianza del SSL de la TSA en base 64: " //$NON-NLS-1$
                                + e,
                        e);
            }
        } else {
        ...
        try {
            this.extensions = extraParams.getProperty("tsaExtensionOid") != null //$NON-NLS-1$
                    && extraParams.getProperty("tsaExtensionValueBase64") != null ? //$NON-NLS-1$
                            new RFC3161RequestExtension[] {
                                    new RFC3161RequestExtension(extraParams.getProperty("tsaExtensionOid"), //$NON-NLS-1$
                                            Boolean.parseBoolean(
                                                    extraParams.getProperty("tsaExtensionCritical", "false")), //$NON-NLS-1$ //$NON-NLS-2$
                                            Base64.decode(extraParams.getProperty("tsaExtensionValueBase64")) //$NON-NLS-1$
                                    ) } : null;
        } catch (final IOException e) {
            throw new IllegalArgumentException(
                    "Las extensiones del sello de tiempo no estan adecuadamente codificadas: " + e, e); //$NON-NLS-1$
        }
    ...
    * @return Nombre del algoritmo de huella digital */
    public static String getDigestAlgorithmName(final String pseudoName) {
        if (pseudoName == null) {
            throw new IllegalArgumentException("El nombre del algoritmo de huella digital no puede ser nulo" //$NON-NLS-1$
            );
        }
